...
C_POTENTIOMETRE::C_POTENTIOMETRE(short int P_Tension_Fourni) {

    Coef_Pourcentage=(pow(2, RESOLUTION)-1)*0.01f;
...
void C_POTENTIOMETRE::Set_Pin_Pot(short P_Pin_Pot){
    
    if (P_Pin_Pot >= 0 && P_Pin_Pot < NOMBRE_DE_POT) {
        Pin_Pot = P_Pin_Pot + A0;
        Valeur_Pot = 0;
...
float C_POTENTIOMETRE::Get_Pourcentage() {
    ...
    Serial << "Entree Pot " << (Pin_Pot - A0) <<" = "<<L_Pourcentage<<" % "<<endl;
