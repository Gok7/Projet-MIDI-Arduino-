...
void C_LISTE_COMMANDES::Set_Commande(short int P_Numero, short int P_code,
        short int P_Valeur_Min, short int P_Valeur_Max) {

    if (P_Numero >= 0 && P_Numero <= Nombres_Commandes) {
        Les_Commandes[P_Numero].Set_Code_MIDI(P_code);
        Les_Commandes[P_Numero].Set_Valeurs(P_Valeur_Min, P_Valeur_Max);
    }
}
